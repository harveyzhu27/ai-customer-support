#!/usr/bin/env python3
"""
Custom Dataset Evaluation Script
Evaluates your assistant with your custom dataset
"""
...
class CustomDatasetEvaluator:
    def __init__(self, dataset_file: str = "custom_evaluation_dataset.json"):
        """Initialize the evaluator with your custom dataset"""
        ...
        try:
            ...
            print(f"✅ Loaded custom dataset: {dataset_file}")
        except FileNotFoundError:
            print(f"❌ Dataset file not found: {dataset_file}")
            print("💡 Run convert_excel_to_evaluation.py first to create your dataset")
            sys.exit(1)
    ...
    def test_question(self, question_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Get response from your assistant (simulating the API call)
    ...
    def get_assistant_response(self, question: str) -> str:
        """
        Get response from your assistant
        This simulates calling your actual assistant API
        """
        try:
            # For demonstration, we'll use OpenAI to simulate responses
            # In practice, you would call your actual assistant API here
            response = openai.ChatCompletion.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "You are Suki, a helpful AI assistant for Aven credit card customers. Provide accurate, helpful responses with specific information about Aven credit card policies, fees, and procedures."},
    ...
    def run_evaluation(self) -> None:
        ...
        """
        print("🚀 Starting Custom Dataset Evaluation")
    ...
    def generate_report(self) -> None:
        ...
        report_filename = f"custom_evaluation_report_{timestamp}.json"
        ...
        csv_filename = f"custom_evaluation_results_{timestamp}.csv"
        df.to_csv(csv_filename, index=False, encoding='utf-8')
        print(f"📊 Results saved to CSV: {csv_filename}")

def main():
    """Main function to run the evaluation"""
    # Get dataset file from command line argument or use default
    dataset_file = sys.argv[1] if len(sys.argv) > 1 else "custom_evaluation_dataset.json"
    
    try:
        evaluator = CustomDatasetEvaluator(dataset_file)
